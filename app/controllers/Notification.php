...
class Notification extends Controller {
    ...
    public function index() {
        ...
        $this->notification = db()->where('notification_id', $this->notification_id)->where('user_id', $this->user->id)->getOne('notifications');
    ...
    private function process_settings_post() {
        ...
        if(!empty($_POST)) {
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                db()->where('notification_id', $this->notification_id)->where('user_id', $this->user->id)->update('notifications', [
                    'name' => $_POST['name'],
    ...
    public function duplicate() {
        ...
        if(!$notification = db()->where('notification_id', $notification_id)->where('user_id', $this->user->id)->getOne('notifications')) {
            redirect('dashboard');
        }
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            $notification_key = md5($this->user->id . $notification->notification_id . $notification->campaign_id . time());
    ...
    public function delete() {
        ...
        /* Make sure the notification is created by the logged in user */
        if(!$notification = db()->where('notification_id', $notification_id)->where('user_id', $this->user->id)->getOne('notifications', ['notification_id', 'campaign_id', 'name'])) {
            redirect('dashboard');
        }
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            db()->where('notification_id', $notification->notification_id)->where('user_id', $this->user->id)->delete('notifications');

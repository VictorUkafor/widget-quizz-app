...
class NotificationCreate extends Controller {
    ...
    public function index() {
        ...
        $user_notifications_total = database()->query("SELECT COUNT(*) AS `total` FROM `notifications` WHERE `user_id` = {$this->user->id}")->fetch_object()->total;
        ...
        if(!empty($_POST)) {
            ...
            if(!db()->where('campaign_id', $_POST['campaign_id'])->where('user_id', $this->user->id)->getValue('campaigns', 'campaign_id')) {
                redirect('notification-create');
            }
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                $notification_key = md5($this->user->id . $_POST['campaign_id'] . $_POST['type'] . time());

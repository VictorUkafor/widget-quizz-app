...
class AdminPlans extends Controller {
    ...
    public function delete() {
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            $result = database()->query("SELECT `id`, `payment_subscription_id` FROM `users` WHERE `plan_id` = {$plan_id} AND `payment_subscription_id` <> ''");

            while($row = $result->fetch_object()) {
                try {
                    (new User())->cancel_subscription($row->id);
                ...
                db()->where('id', $row->id)->update('users', ['plan_id' => 'custom']);

                /* Clear the cache */
                \Altum\Cache::$adapter->deleteItemsByTag('id=' . $row->id);
            }

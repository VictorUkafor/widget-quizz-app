...
class Cron extends Controller {
    ...
    private function users_deletion_reminder() {
        ...
        $result = database()->query("
            SELECT `id`, `name`, `email`, `language` FROM `users` WHERE `plan_id` = 'free' AND `last_activity` < '{$past_date}' AND `user_deletion_reminder` = 0 AND `type` = 0 LIMIT 25
        ");
        ...
        while($user = $result->fetch_object()) {
            ...
            /* Update user */
            db()->where('id', $user->id)->update('users', ['user_deletion_reminder' => 1]);

            if(DEBUG) {
                if(settings()->users->user_deletion_reminder) echo sprintf('User deletion reminder email sent for id %s', $user->id);
            }
        }
    ...
    private function auto_delete_inactive_users() {
        ...
        /* Get the users that need to be reminded */
        $result = database()->query("
            SELECT `id`, `name`, `email`, `language` FROM `users` WHERE `plan_id` = 'free' AND `last_activity` < '{$past_date}' AND `user_deletion_reminder` = 1 AND `type` = 0 LIMIT 25
        ");
        ...
        while($user = $result->fetch_object()) {
            ...
            if(DEBUG) {
                echo sprintf('User deletion for inactivity id %s', $user->id);
            }
        }
    ...
    private function notifications_logs_cleanup() {
        ...
        $result = database()->query("SELECT `id`, `plan_settings` FROM `users` WHERE `status` = 1");
        ...
        while($user = $result->fetch_object()) {
            ...
            if(DEBUG) {
                echo sprintf('Track notifications cleanup done for id %s', $user->id);
            }
        }
    ...
    private function users_plan_expiry_reminder() {
        ...
        while($user = $result->fetch_object()) {
            ...
            /* Update user */
            db()->where('id', $user->id)->update('users', ['plan_expiry_reminder' => 1]);

            if(DEBUG) {
                echo sprintf('Email sent for id %s', $user->id);
            }
        }

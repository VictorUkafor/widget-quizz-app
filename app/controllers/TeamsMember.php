...
class TeamsMember extends Controller {
    ...
    public function index() {
        ...
        $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `teams_members` WHERE (`user_id` = {$this->user->id} OR `user_email` = '{$this->user->email}') {$filters->get_sql_where()}")->fetch_object()->total ?? 0;
        ...
        $teams_member_result = database()->query("
            ...
            WHERE 
                  (`teams_members`.`user_id` = {$this->user->id} 
    ...
    public function delete() {
        ...
        if(!$team = db()->where('team_id', $team_id)->where('user_id', $this->user->id)->getOne('teams')) {
            redirect('teams');
        }

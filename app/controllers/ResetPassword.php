...
class ResetPassword extends Controller {
    ...
    public function index() {
        ...
        $user = db()->where('email', $email)->where('lost_password_code', $lost_password_code)->getOne('users', ['id', 'name', 'password']);

        if($user->id < 1 || mb_strlen($lost_password_code) < 1) redirect();
        ...
        if(!empty($_POST)) {
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                Logger::users($user->id, 'reset_password.success');

...
class Teams extends Controller {
    ...
    public function index() {
        ...
        $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `teams` WHERE `user_id` = {$this->user->id} {$filters->get_sql_where()}")->fetch_object()->total ?? 0;
        ...
        $teams_result = database()->query("
            ...
            WHERE `teams`.`user_id` = {$this->user->id} {$filters->get_sql_where('teams')} 

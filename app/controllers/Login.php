...
class Login extends Controller {
    ...
    public function index() {
        ...
        if($method == 'one-time-login-code') {
            ...
            $user = db()->where('one_time_login_code', $one_time_login_code)->getOne('users', ['id', 'password', 'name', 'status', 'language']);
            ...
            (new User())->login_aftermath_update($user->id);

            /* Remove one time login */
            db()->where('id', $user->id)->update('users', ['one_time_login_code' => null]);
            ...
            if(\Altum\Language::$name == $user->language) {
                ...
                redirect(\Altum\Language::$active_languages[$user->language] . '/' . $redirect);
            }
        }
        ...
        if(!empty($_POST)) {
            ...
            /* Try to get the user from the database */
            $user = db()->where('email', $_POST['email'])->getOne('users', ['id', 'email', 'name', 'status', 'password', 'token_code', 'twofa_secret', 'language']);
            ...
            if(!$user) {
                ...
                if($user->status != 1) {
                    ...
                    if(!password_verify($_POST['password'], $user->password)) {
                        Logger::users($user->id, 'login.wrong_password');
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors() && !Alerts::has_infos()) {
                ...
                if(isset($_POST['rememberme'])) {
                    ...
                    if(empty($user->token_code)) {
                        ...
                        db()->where('id', $user->id)->update('users', ['token_code' => $token_code]);
                    }

                    setcookie('id', $user->id, time()+60*60*24*30, COOKIE_PATH);
    ...
    private function process_social_login($email, $name, $redirect, $method) {
        ...
        if($user = db()->where('email', $email)->getOne('users', ['id', 'email', 'password', 'lost_password_code', 'language'])) {

            /* Make sure the user has a password set before letting the user login */
            (new User())->verify_null_password($user->id, $user->email, $user->password);

            $_SESSION['id'] = $user->id;
            $_SESSION['user_password_hash'] = md5($user->password);

            (new User())->login_aftermath_update($user->id, $method);
        ...
        /* Create a new account */
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                /* Log the action */
                Logger::users($registered_user['id'], 'register.' . $method . '.success');

...
class Pay extends Controller {
    ...
    public function index() {
        ...
        if(!empty($_POST) && !$this->return_type) {
            ...
            if(settings()->payment->codes_is_enabled && isset($_POST['code'])) {
                ...
                if($this->code) {
                    if(db()->where('user_id', $this->user->id)->where('code_id', $this->code->code_id)->has('redeemed_codes')) {
                        $this->code = null;
                    }
            ...
            if($this->plan->trial_days && !$this->user->plan_trial_done && !isset($_GET['trial_skip'])) {
                ...
                if($this->user->plan_id != $this->plan->plan_id) {
                    try {
                        (new User())->cancel_subscription($this->user->id);
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                if($this->plan->trial_days && !$this->user->plan_trial_done && !isset($_GET['trial_skip'])) {
                    ...
                    /* Database query */
                    db()->where('id', $this->user->id)->update('users', [
                    ...
                    /* Clear the cache */
                    \Altum\Cache::$adapter->deleteItemsByTag('id=' . $this->user->id);
    ...
    private function paypal() {
        ...
        $custom_id = $this->user->id . '&' . $this->plan_id . '&' . $_POST['payment_frequency'] . '&' . $base_amount . '&' . $code . '&' . $discount_amount . '&' . json_encode($this->applied_taxes_ids);
...
//                    'start_time' => (new \DateTime())->modify('+30 seconds')->format(DATE_ISO8601),
    ...
    private function offline_payment() {
        ...
        /* Other vars */
        $payment_id = md5($this->user->id . $this->plan_id . $_POST['payment_type'] . $_POST['payment_frequency'] . $this->user->email . Date::$date);
    ...
    public function code() {
        ...
        if(db()->where('user_id', $this->user->id)->where('code_id', $code->code_id)->has('redeemed_codes')) {
    ...
    private function redirect_pay_thank_you() {
        ...
        $thank_you_url_parameters .= '&user_id=' . $this->user->id;

...
class ApiPayments extends Controller {
    ...
    private function get_all() {
        ...
        $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `payments` WHERE `user_id` = {$this->api_user->id}")->fetch_object()->total ?? 0;
    ...
    private function get() {
        ...
        $payment = db()->where('id', $id)->where('user_id', $this->api_user->id)->getOne('payments');

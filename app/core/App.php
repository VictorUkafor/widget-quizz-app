    ...
    public function __construct() {
        ...
        if(Authentication::check()) {
            ...
            if(!$user->plan || ($user->plan && ((new \DateTime()) > (new \DateTime($user->plan_expiration_date)) && $user->plan_id != 'free') || !$user->plan->status)) {
                ...
                /* Clear the cache */
                \Altum\Cache::$adapter->deleteItemsByTag('id=' .  Authentication::$id);

                /* Make sure to redirect the person to the payment page and only let the person access the following pages */
                if(!in_array(Router::$controller_key, ['plan', 'pay', 'pay-billing', 'pay-thank-you', 'account', 'account-plan', 'account-payments', 'account-logs', 'account-delete', 'referrals', 'account-api', 'account-redeem-code', 'logout', 'register', 'teams-system', 'teams-member', 'teams-members']) && Router::$path != 'admin') {
                    redirect('plan/new');
                }
            }
            ...
            if(!$user->last_activity || (new \DateTime($user->last_activity))->modify('+5 minutes') < (new \DateTime())) {
                (new User())->update_last_activity(Authentication::$id);
            }
            ...
            if(isset($_GET['set_language']) && array_key_exists($_GET['set_language'], Language::$languages) && Language::$name != $user->language) {
                db()->where('id', Authentication::$id)->update('users', ['language' => $_GET['set_language']]);

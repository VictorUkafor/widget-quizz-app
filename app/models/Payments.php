...
class Payments extends Model {
    ...
    public function webhook_process_payment($payment_processor, $external_payment_id, $payment_total, $payment_currency, $id, $plan_id, $payment_frequency, $code, $discount_amount, $base_amount, $taxes_ids, $payment_type, $payment_subscription_id, $payer_email, $payer_name) {
        ...
        if(!empty($user->payment_subscription_id) && ($payment_subscription_id && $user->payment_subscription_id != $payment_subscription_id)) {
            try {
                (new User())->cancel_subscription($id);
    ...
    public function codes_payment_check($code, $user) {
        ...
        if($codes_code) {
            ...
            if(!db()->where('user_id', $user->id)->where('code_id', $codes_code->code_id)->has('redeemed_codes')) {
    ...
    public function affiliate_payment_check($payment_id, $payment_total, $user) {
        if(\Altum\Plugin::is_active('affiliate') && settings()->affiliate->is_enabled && $user->referred_by) {
            if((settings()->affiliate->commission_type == 'once' && !$user->referred_by_has_converted) || settings()->affiliate->commission_type == 'forever') {
                $referral_user = db()->where('id', $user->referred_by)->getOne('users', ['id', 'email', 'status']);
                ...
                if($referral_user && $referral_user->status == 1) {
                    ...
                    db()->where('id', $user->id)->update('users', ['referred_by_has_converted' => 1]);
                }
            }
        }
    }

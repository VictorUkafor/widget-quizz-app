...
class User extends Model {
    ...
    public function get_user_by_user_id($id) {
        ...
        $cache_instance = \Altum\Cache::$adapter->getItem('user?id=' . $id);
        ...
        if(is_null($cache_instance->get())) {
            ...
            if($data) {
                ...
                \Altum\Cache::$adapter->save(
                    $cache_instance->set($data)->expiresAfter(CACHE_DEFAULT_SECONDS)->addTag('users')->addTag('id=' . $data->id)
                );
            }
    ...
    public function get_user_by_user_id_and_token_code($id, $token_code) {

        /* Try to check if the store posts exists via the cache */
        $cache_instance = \Altum\Cache::$adapter->getItem('user?id=' . md5($id) . '&token_code=' . $token_code);

        /* Set cache if not existing */
        if(is_null($cache_instance->get())) {

            /* Get data from the database */
            $data = db()->where('id', $id)->where('token_code', $token_code)->getOne('users');
    ...
    public function delete($id) {
        ...
        if(settings()->webhooks->user_delete) {
            ...
            $user = db()->where('id', $id)->getOne('users', ['id', 'email', 'name']);
        ...
        /* Clear the cache */
        \Altum\Cache::$adapter->deleteItemsByTag('id=' . $id);

    }

    public function update_last_activity($id) {
        db()->where('id', $id)->update('users', ['last_activity' => \Altum\Date::$date]);
    }

    public function verify_null_password($id, $email, $password) {
        if(empty($password)) {
            $lost_password_code = $lost_password_code ?? md5($email . microtime());
            db()->where('id', $id)->update('users', ['lost_password_code' => $lost_password_code]);
    ...
    public function create(
        ...
        $referred_by = null;
        if(!$is_admin_created && isset($_COOKIE['referred_by']) && $user = db()->where('referral_key', $_COOKIE['referred_by'])->getOne('users', ['id', 'referral_key'])) {
            $referred_by = $user->id;
        }

        /* Add the user to the database */
        $registered_id = db()->insert('users', [
            'password' => $password,
            'email' => $email,
            'name' => $name,
            'billing' => $billing,
    ...
    */
    public function login_aftermath_update($id, $method = 'default') {
        ...
        Logger::users($id, 'login.' . $method . '.success');
    ...
    public function cancel_subscription($id) {
        ...
        $user = db()->where('id', $id)->getOne('users', ['id', 'payment_subscription_id', 'payment_processor']);
        ...
        /* Database query */
        db()->where('id', $user->id)->update('users', ['payment_subscription_id' => '']);

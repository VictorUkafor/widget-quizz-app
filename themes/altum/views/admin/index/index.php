...
<div class="mb-5">
    ...
    <?php $result = database()->query("SELECT * FROM `users` ORDER BY `id` DESC LIMIT 5"); ?>
                ...
                <tr>
                    <td class="text-nowrap">
                        <div class="d-flex">
                            ...
                            <div class="d-flex flex-column">
                                <div>
                                    <a href="<?= url('admin/user-view/' . $row->id) ?>" <?= $row->type == 1 ? 'class="font-weight-bold" data-toggle="tooltip" title="' . l('admin_users.main.type_admin') . '"' : null ?>><?= $row->name ?></a>
                                </div>
                    ...
                    <td>
                        <div class="d-flex justify-content-end">
                            <?= include_view(THEME_PATH . 'views/admin/users/admin_user_dropdown_button.php', ['id' => $row->id]) ?>
                        </div>
                    </td>
                </tr>
            <?php endwhile ?>
...
<?php if(in_array(settings()->license->type, ['Extended License', 'extended'])): ?>
    <?php $result = database()->query("SELECT `payments`.*, `users`.`name` AS `user_name`, `users`.`email` AS `user_email` FROM `payments` LEFT JOIN `users` ON `payments`.`user_id` = `users`.`id` ORDER BY `id` DESC LIMIT 5"); ?>
                    ...
                    <?php while($row = $result->fetch_object()): ?>
                        ...
                        <tr>
                            <td class="text-nowrap">
                                <div class="d-flex flex-column">
                                    <div>
                                        <a href="<?= url('admin/user-view/' . $row->id) ?>"><?= $row->user_name ?></a>
                                    </div>
